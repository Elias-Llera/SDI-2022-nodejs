...
module.exports = function (app, songsRepository, commentsRepository) {
    ...
    function canBuySong(user, songId, functionCallback){
        let author_filter = {$and: [{"_id": songId}, {"author": user}]};
        let bought_filter = {$and: [{"songId": songId}, {"user": user}]}

        songsRepository.getSongs(author_filter, {})
            .then(songs => {
                if (songs == null || songs.length > 0) {
                    functionCallback(false);
                } else {
                    songsRepository.getPurchases(bought_filter,{})
                        .then( songs => {
                            if (songs == null || songs.length > 0) {
                                functionCallback(false);
                            } else {
    ...
    app.get('/songs/buy/:id', function (req, res) {
        ...
        canBuySong(req.session.user, songId, (canBuy) => {
            if (canBuy) {
                let shop = {
    ...
    app.get("/songs/delete/:id", function(req, res){
       let filter = { _id:ObjectId(req.params.id)};
       songsRepository.deleteSong(filter, {})
           .then(result => {
                if(result == null || result.deleteCount == 0) {
                    res.send("No se ha podido eliminar el registro.");
                } else {
                    res.redirect("/publications");
                }
           }).catch(error => {
               res.send("Se ha producido un error al intentar eliminar la cación " + error);
           });
    });

    app.get('/songs/:id', function (req, res) {
        let songId = ObjectId(req.params.id);
        let songFilter = {_id: songId};
        let commentsFilter = {song_id : songId};
        let options = {};

        songsRepository
            .findSong(songFilter, options)
            .then(song =>
                commentsRepository
                    .getComments(commentsFilter, options)
                    .then(comments =>
                        canBuySong(req.session.user, songId, (canBuy) =>{
